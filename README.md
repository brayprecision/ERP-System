...
- **Maximize** — The main window’s **minimum size** is limited to the **primary display work area** (not a fixed 1024×768), and the **minimum floors** (400px and up to **1024×768**) never exceed that work area. On short displays (e.g. **1366×768** with a panel), a larger minimum used to prevent the window manager from honoring **maximize**; that behavior is fixed in **`electron/main.js`**. On **Linux**, filling the screen uses **`screen.workArea`** (not **`maximize()`**) so the window does not extend **under the taskbar**; **default** **`window.maximized`** is **`true`** in **`electron-store`** for new installs (existing **`~/.config/.../bperp-config.json`** may still have **`maximized: false`**). If **maximize** or **resize** misbehaves (including on **X11**), check session type with **`echo $XDG_SESSION_TYPE`**. From the repo root you can try **`BPERP_OZONE_PLATFORM=x11 npm start`** to force Chromium’s **X11** path on bad **Wayland** sessions; packaged installs can use the same env var when launching **BPERP** from a terminal or a modified `.desktop` **`Exec=`** line.
