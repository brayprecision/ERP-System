...
- **Electron — Linux work-area fill (Cinnamon / X11)** — **`BrowserWindow.maximize()`** often sizes to the **full screen**, so the **bottom** sits **under the panel**. On **Linux**, “maximized” is implemented with **`setBounds(display.workArea)`** instead of **`maximize()`** (startup when **`window.maximized`**, **IPC**, and the **titlebar** `maximize` event). **`is-maximized`** treats **fill** as maximized. **Default** **`window.maximized`** is **`true`** for **new** configs so the app **opens filled**; existing **`bperp-config.json`** keeps a saved **`false`**.

- **Electron — maximize / resize (Linux)** — **Minimum height** (and width) could equal the **entire work area** (e.g. short 768px-tall panels), so **`minHeight === usable height`** and the window **could not be resized vertically**; **maximize** also behaved badly with some window managers. **`minWidth` / `minHeight`** are now capped so they stay **strictly below** the primary work width/height (at least **1px** slack) while keeping the same **1024×768**-style caps. Post-**show** on Linux, **`setMinimumSize`** is applied again with that logic. The experimental post-maximize **snap-to-workArea** path was **removed** (it fought the WM). Optional: launch with **`BPERP_OZONE_PLATFORM=x11`** to force Chromium’s **X11** path on problematic **Wayland** sessions (see README).
...
- **Inventory → Inspection** — Gage profiles, calibration dates, and PDF/image attachments in **`localStorage`** (`bperp_inspection_tools`; documents as base64). **Sync reminders** adds **misc tasks** when calibrations are due (within 30 days) or overdue. **Inspection** workcenter links to this inventory. Optional SQLite migration path: **`inspection_tool_assets`** / **`inspection_tool_documents`**, REST **`/api/inspection-inventory/*`**, disk uploads under **`backend/uploads/inspection-tools/`**, server **`Calibration`** tasks — see **`backend/README.md`**.
